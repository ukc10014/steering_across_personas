...
3. **Seed 2 — the noise floor for every threshold in §5.1.** Change only `--seed`, both
   stages, reusing the released DPO data and the pinned SFT corpus. (Reusing the corpus is
   correct *here* — this arm is meant to reproduce the character.)

   Revision 1 and plan §4 justified this arm as *rig validation*, on the grounds that its
   expected answer is already known. That is step 1's job, and step 1 does it better by
   comparing against the actual released adapter. Seed 2's real value is different and
   larger: **it is the only thing that says how far apart two runs of the SAME constitution
   land**, and every threshold in §5.1 is meaningless without it. If two `impulsiveness`
   seeds agree only at cos 0.60, then a sham at 0.45 is not obviously distinguishable from a
   real arm, and the "≥ 0.45 trained-like" line has to move. Score seed 2 on all three
   statistics — cos, retention k, and the primary contrast — and report the seed-to-seed
   value beside each band as the ceiling.

   Two limitations to state when reporting it. (i) **n = 1.** One pair gives a point estimate
   of the ceiling with no seed-level uncertainty; the question bootstrap around it is
   measurement noise, not seed noise. (ii) The SFT corpus is held fixed across the two seeds
   and was generated by seed 1's distilled model, so seed 2 inherits seed 1's introspective
   content and the measured agreement is an **upper** bound on true pipeline reproducibility.
   Both push in the direction of a generous ceiling, which makes "the sham is trained-like"
   harder to claim and is the conservative direction for the deflationary reading.

   Extract the **DPO-stage adapter for both seeds**, not only the merged one — §3.3 makes the
   DPO stage the primary comparator, so the noise floor has to exist at that stage too.
   The merged artifact additionally carries `B_dpo@A_sft + B_sft@A_dpo` cross terms from
   peft's factor-space combination (`scripts/check_peft_merge.py`), which the DPO-stage
