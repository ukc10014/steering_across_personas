...
3. **The introspection SFT stage leaks the character back in** (§3.3) — verified in the OCT
   source, not assumed. This changes what the sham can be.
...
**That does not produce a sham.** Traced in the OCT source
(`github.com/maiush/OpenCharacterTraining`, `character/introspection/`), the SFT stage is
character-specific in two independent ways:

1. **The SFT corpus is generated by the constitution's own DPO adapter.**
   `self_reflection.py:80` loads `LORA_PATH/{name}-distillation/{constitution}` as a vLLM
   `LoRARequest` and generates with it. The transcripts are the distilled character talking
   about itself.
2. **The generating system prompt contains the constitution's trait list verbatim.**
   `self_reflection.py:100–117` reads `constitutions/few-shot/{constitution}.jsonl`, joins
   `cons["trait"].unique()` into a numbered `{TRAITS}` block and interpolates it into the
   system prompt. The character is injected as *text*, not merely as a model bias.

`introspection/data.py` then writes one `sft_data/{model}/{constitution}.jsonl` per
constitution (≈3,000 examples: 1,000 self-reflection + 2,000 self-interaction, `--N 1000`
each, default and leading).

So reusing released SFT data gives **a DPO ablation followed by genuine character SFT** —
the character is restored, in text, at the strongest point in the pipeline. This is fatal to
revision 1's design and is why the spec now has a §3.3.
...
- **Secondary (F):** the full pipeline, with the SFT corpus **regenerated** from the sham's
  own DPO adapter, and the `{TRAITS}` block shammed to match the arm (S1: traits sampled
  from the mixed constitution pool; S2: traits resampled per example across all
  constitutions, so no coherent character is nameable). Costs one vLLM generation pass per
  arm (~3,000 completions at ≤2,048 new tokens) plus the SFT stage.
...
| *(F arms only)* regenerate SFT corpus | ~3,000 completions, ≤2,048 new tokens | 1–3 h GPU |
