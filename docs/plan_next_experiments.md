...
> **sham**. The released `sft_data/<model>/<constitution>.jsonl` is character-specific twice
> over — generated by that constitution's own DPO adapter, under a system prompt that
> interpolates the constitution's trait list verbatim — so a sham that reuses it is a DPO
> ablation followed by genuine character SFT. See spec §3.3, which moves the sham's primary
> comparison to the DPO stage for exactly this reason.
